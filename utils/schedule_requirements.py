...
class ScheduleRequirements:
    ...
    SPECIAL_SUBJECT_RANGES = {
        ...
        "T.C. İnkılap Tarihi ve Atatürkçülük": [(8, 8), (12, 12)]  # Grade 8 in middle school, grade 12 in high school
    }
    ...
    @classmethod
    def validate_schedule(cls, grade, mandatory_subjects, optional_subjects=None, free_activities_hours=0):
        ...
        for subject, required_hours in required_subjects.items():
            ...
            if mandatory_subjects[subject] != required_hours:
                return False, f"Mandatory subject '{subject}' should have {required_hours} hours, but has {mandatory_subjects[subject]}"
        ...
        if total_hours > max_hours:
            return False, f"Total hours ({total_hours}) exceed the limit ({max_hours}) for grade {grade}"
        ...
        if 9 <= grade <= 12:
            ...
            if total_optional_hours > max_optional_hours:
                return False, f"Optional subject hours ({total_optional_hours}) exceed the limit ({max_optional_hours}) for grade {grade}"
