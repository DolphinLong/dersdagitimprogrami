    ...
    def save_class(self):
        ...
        if not grade_text or not section_text:
            msg = create_styled_message_box(
                self, "Hata", "Lütfen tüm alanları doldurun.", QMessageBox.Warning
            )
            msg.exec_()
            return
        ...
        try:
            ...
            msg = create_styled_message_box(
                self, "Hata", "Lütfen geçerli bir seviye numarası girin.", QMessageBox.Warning
            )
            msg.exec_()
            return
        ...
        try:
            if self.class_obj:
                ...
                if success:
                    msg = create_styled_message_box(
                        self, "Başarılı", "Sınıf bilgileri başarıyla güncellendi."
                    )
                    msg.exec_()
                    self.accept()
                else:
                    msg = create_styled_message_box(
                        self,
                        "Hata",
                        "Sınıf bilgileri güncellenirken bir hata oluştu.",
            ...
            msg = create_styled_message_box(
                self, "Hata", f"Sınıf kaydedilirken bir hata oluştu: {str(e)}", QMessageBox.Critical
            )
            msg.exec_()
