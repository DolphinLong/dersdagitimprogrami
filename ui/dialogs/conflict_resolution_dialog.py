...
from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QTableWidget, QTableWidgetItem, QHeaderView, QTextEdit, QFrame, QMessageBox
...
class ConflictResolutionDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        desc_label = QLabel(f"Toplam {len(self.conflicts)} çakışma bulundu. Çakışmaları inceleyin ve çözümleyin.")
    ...
    def populate_conflicts(self):
        ...
        for row, conflict in enumerate(self.conflicts):
            ...
            # Day
            day_text = day_names[entry1_day] if entry1_day is not None and entry1_day < len(day_names) else f"Gün {entry1_day}"
            day_item = QTableWidgetItem(day_text)
            day_item.setTextAlignment(Qt.AlignCenter)  # type: ignore
            
            # Time slot
            time_text = time_slot_labels[entry1_time_slot] if entry1_time_slot is not None and entry1_time_slot < len(time_slot_labels) else f"Saat {entry1_time_slot}"
    ...
    def resolve_conflict(self):
        ...
        if resolved:
            msg = create_styled_message_box(
                self,
                "Başarılı",
                "Çakışma başarıyla çözüldü. Program yenileniyor."
            )
            msg.exec_()
