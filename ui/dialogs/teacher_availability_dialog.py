...
class TeacherAvailabilityDialog(QDialog):
    ...
    def __init__(self, parent=None, teacher=None):
        ...
        self.setWindowTitle(f"Öğretmen Uygunluk Ayarları - {teacher.name if teacher else 'Yeni Öğretmen'}")
    ...
    def setup_ui(self):
        ...
        title_label = QLabel(f"Öğretmen Uygunluk Ayarları - {self.teacher.name if self.teacher else 'Yeni Öğretmen'}")
        ...
        # Instruction label
        instruction_label = QLabel("Aşağıdaki tabloda öğretmenin uygun olduğu saatleri işaretleyin. Yeşil: Uygun, Kırmızı: Uygun Değil")
    ...
    def save_availability(self):
        ...
        if not self.teacher:
            msg = create_styled_message_box(self, "Hata", "Öğretmen bilgisi bulunamadı.", QMessageBox.Warning)
            msg.exec_()
            return
        ...
        try:
            ...
            for (day, time_slot), is_available in self.availability_data.items():
                if db_manager.set_teacher_availability(self.teacher.teacher_id, day, time_slot, is_available):
                    success_count += 1
