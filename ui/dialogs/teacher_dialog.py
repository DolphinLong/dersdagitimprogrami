...
class TeacherDialog(QDialog):
    ...
    def open_availability_dialog(self):
        ...
        if not self.teacher:
            ...
            if not teacher_id:
                msg = create_styled_message_box(
                    self, "Hata", "Öğretmen eklenirken bir hata oluştu.", QMessageBox.Critical
                )
                msg.exec_()
                return
    ...
    def save_teacher(self):
        ...
        try:
            if self.teacher:
                ...
                if success:
                    msg = create_styled_message_box(
                        self, "Başarılı", "Öğretmen bilgileri başarıyla güncellendi."
                    )
                    msg.exec_()
                    self.accept()
                else:
                    msg = create_styled_message_box(
                        self,
                        "Hata",
                        "Öğretmen bilgileri güncellenirken bir hata oluştu.",
            ...
            msg = create_styled_message_box(
                self,
                "Hata",
                f"Öğretmen kaydedilirken bir hata oluştu: {str(e)}",
                QMessageBox.Critical,
            )
            msg.exec_()
