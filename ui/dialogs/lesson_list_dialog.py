...
from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QMessageBox, QTableWidget, QTableWidgetItem, QHeaderView
...
class LessonListDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        if school_type:
            ...
            school_type_label.setStyleSheet("font-size: 14px; color: #3498db; font-weight: bold; margin: 5px;")
    ...
    def load_lessons(self):
        ...
        for row, lesson in enumerate(all_lessons):
            ...
            self.lessons_table.setItem(row, 2, QTableWidgetItem(str(weekly_hours) if weekly_hours else "0"))
    ...
    def get_predefined_subjects_for_school_type(self, school_type):
        ...
        if school_type == "İlkokul":
            ...
            optional_subjects = [
                "Oyun ve Fiziki Etkinlikler", "Değerler Eğitimi", "Yaşam Becerileri",
                "Çevre Eğitimi", "Okuma Yazma Hazırlığı", "Temel Matematik Becerileri"
            ]
            ...
            optional_subjects = [
                ...
                "Medya Okuryazarlığı", "Dijital Vatandaşlık", "Girişimcilik", "Finansal Okuryazarlık",
                ...
                "Drama", "Tiyatro", "Müze Eğitimi", "Görsel Sanatlar Uygulamaları",
                "Müzik Uygulamaları", "Çalgı Eğitimi", "Koro", "İkinci Yabancı Dil",
                "Almanca", "Fransızca", "Rusça", "Arapça", "Çince", "İspanyolca",
                "İtalyanca", "Japonca"
            ]
            subjects.update(optional_subjects)
            ...
            optional_subjects = [
                "İleri Matematik", "Matematik Uygulamaları", "İleri Fizik", "Fizik Uygulamaları",
                "İleri Kimya", "Kimya Uygulamaları", "İleri Biyoloji", "Biyoloji Uygulamaları",
                "Astronomi ve Uzay Bilimleri", "Çevre Bilimi", "Genetik ve Biyoteknoloji",
                "Sağlık Bilgisi ve İlk Yardım", "Spor Bilimleri", "Beslenme ve Diyetetik",
                "Psikoloji", "Sosyoloji", "Mantık", "Felsefe Tarihi", "Karşılaştırmalı Edebiyat",
                "Dil ve Anlatım Uygulamaları", "Türk Dili", "Türk Edebiyatı",
                "Dünya Edebiyatından Seçmeler", "Coğrafya Uygulamaları", "Tarih Uygulamaları",
                "Sanat Tarihi", "Müze Bilimi", "Arkeoloji", "Antropoloji", "Ekonomi",
                "İşletme", "Muhasebe", "Hukuk", "Uluslararası İlişkiler", "Siyaset Bilimi",
                "İstatistik", "Bilgisayar Bilimleri", "Programlama", "Web Tasarımı",
                "Grafik Tasarım", "Endüstriyel Tasarım", "Mimarlık", "Mühendislik Uygulamaları",
                "Yapay Zeka ve Makine Öğrenmesi", "Veri Bilimi", "Siber Güvenlik",
                "Oyun Geliştirme", "Mobil Uygulama Geliştirme"
            ]
            subjects.update(optional_subjects)
            ...
            optional_subjects = [
                ...
                "İleri Kimya", "Kimya Uygulamaları", "İleri Biyoloji", "Biyoloji Uygulamaları",
                "Astronomi ve Uzay Bilimleri", "Çevre Bilimi", "Genetik ve Biyoteknoloji",
                "Bilgisayar Bilimleri", "Programlama", "Yapay Zeka ve Makine Öğrenmesi",
                "Veri Bilimi", "Mühendislik Uygulamaları", "İstatistik"
            ]
            subjects.update(optional_subjects)
            ...
            for grade in range(9, 13):
                mandatory_subjects = ScheduleRequirements.SOSYAL_BILIMLER_LISESI_SUBJECTS.get(grade, {})
                subjects.update(mandatory_subjects.keys())
        ...
        common_subjects = [
            "Rehberlik", "Seçmeli Ders", "Seçmeli 1", "Seçmeli 2", "Seçmeli 3"
        ]
    ...
    def delete_lesson(self):
        ...
        # Confirm deletion
        msg = create_styled_message_box(self, "Ders Sil", f"'{lesson_name}' dersini silmek istediğinizden emin misiniz?", QMessageBox.Warning)
        ...
        if msg.exec_() == QMessageBox.Yes:
            try:
                ...
                if success:
                    ...
                    info_msg = create_styled_message_box(self, "Başarılı", "Ders silindi", QMessageBox.Information)
                    info_msg.exec_()
                else:
                    msg = create_styled_message_box(self, "Hata", "Ders silinirken bir hata oluştu.", QMessageBox.Critical)
                    msg.exec_()
            except Exception as e:
                msg = create_styled_message_box(self, "Hata", f"Ders silinirken bir hata oluştu: {str(e)}", QMessageBox.Critical)
                msg.exec_()