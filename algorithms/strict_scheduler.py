...
class StrictScheduler:
    ...
    def generate_schedule(self) -> List[Dict]:
        ...
        for class_idx, class_obj in enumerate(classes, 1):
            ...
            for lesson in class_lessons:
                print(f"   • {lesson['lesson_name']}: {lesson['weekly_hours']} saat/hafta (Öğretmen: {lesson['teacher_name']})")
            ...
            for lesson_info in class_lessons:
                ...
                if scheduled < lesson_info['weekly_hours']:
                    ...
                    print(f"   ❌ UYARI: {lesson_info['lesson_name']} tam yerleştirilemedi! ({scheduled}/{lesson_info['weekly_hours']})")
        ...
        if failed_lessons:
            ...
            for fail in failed_lessons:
                print(f"   • {fail['class']} - {fail['lesson']} ({fail['teacher']}): {fail['scheduled']}/{fail['required']} saat")
            print(f"\n💡 Öneriler:")
    ...
    def _schedule_lesson_strict(
        ...
        scheduled_hours = self._try_block_distribution(
            class_obj.class_id, teacher_id, lesson_id, 
            weekly_hours, time_slots_count, classrooms, used_days, block_size=2
        )
        ...
        if scheduled_hours < weekly_hours:
            ...
            scheduled_hours += self._try_single_slots(
                class_obj.class_id, teacher_id, lesson_id,
                remaining, time_slots_count, classrooms, used_days
            )
