...
try:
    from algorithms.csp_solver import CSPVariable, CSPSolver, create_schedule_constraints
    CSP_AVAILABLE = True
except ImportError:
    CSP_AVAILABLE = False
...
try:
    from algorithms.local_search import SimulatedAnnealing, ScheduleNeighborGenerator, adaptive_backtrack_limit
...
class HybridOptimalScheduler:
    ...
    def _optimize_with_annealing(self, schedule: List[Dict], 
                                config: Dict) -> List[Dict]:
        """Simulated Annealing ile iyileştirme"""
        
        # Komşu üreteç
        neighbor_gen = ScheduleNeighborGenerator(
            self.db_manager, config['time_slots_count']
        )
        ...
        optimized = sa.optimize(
            schedule,
            evaluate,
            neighbor_gen.generate_neighbor,
            check_constraints
        )
        
        return optimized
    
    def _final_validation(self, schedule: List[Dict], 
                         config: Dict) -> List[Dict]:
    ...
    def _resolve_conflicts(self, schedule: List[Dict], 
                          conflicts: List[Dict],
                          config: Dict) -> List[Dict]:
        """Çakışmaları çözmeye çalış"""
                    ...
                    for day in range(5):
                        for slot in range(config['time_slots_count']):
                            ...
                            if self._is_slot_available(
                                schedule, entry['class_id'], 
                                entry['teacher_id'], day, slot
                            ):
                                # Taşı
                                entry['day'] = day
    ...
    def _is_slot_available(self, schedule: List[Dict],
