...
class SimplePerfectScheduler:
    ...
    def generate_schedule(self) -> List[Dict]:
        """Program oluştur"""
        print("\n" + "="*80)
        print("🎯 SIMPLE PERFECT SCHEDULER - Pragmatik ve Etkili")
        print("="*80)
        ...
        print(f"   {len(all_needs)} farklı ders ataması")
        ...
        for idx, need in enumerate(all_needs):
            if (idx + 1) % 10 == 0:
                print(f"   📊 İlerleme: {idx+1}/{len(all_needs)} ders")
        ...
        if failed:
            ...
            for f in failed[:5]:
                print(f"   • {f['class_name']} - {f['lesson_name']}: {f['scheduled']}/{f['weekly_hours']}")
        else:
        ...
        print(f"✅ {saved} kayıt tamamlandı")
        
        return self.schedule_entries
    
    def _schedule_lesson(
        self, need: Dict, time_slots_count: int, 
        classrooms: List, max_attempts: int = 5
    ) -> int:
        """
        ...
        if weekly_hours >= 6:
            ...
            if scheduled < weekly_hours:
                ...
                scheduled += self._try_singles(
                    class_id, teacher_id, lesson_id,
                    remaining, time_slots_count, classrooms,
            ...
            scheduled, used_days = self._try_blocks_strict(
                class_id, teacher_id, lesson_id,
                1, time_slots_count, classrooms, 2
            )
            # Sonra 1 tekli (FARKLI bir güne)
            if scheduled == 2:
                scheduled += self._try_singles(
    ...
    def _try_blocks_strict(
        self, class_id: int, teacher_id: int, lesson_id: int,
        num_blocks: int, time_slots_count: int, classrooms: List,
        block_size: int
    ) -> tuple:
        """
    ...
    def _try_blocks(
        self, class_id: int, teacher_id: int, lesson_id: int,
        total_hours: int, time_slots_count: int, classrooms: List,
        block_size: int
    ) -> int:
    ...
    def _try_singles(
        self, class_id: int, teacher_id: int, lesson_id: int,
        hours_needed: int, time_slots_count: int, classrooms: List,
    ...
    def _can_place_all(
        ...
        day: int, slots: List[int], lesson_id: int = None
    ) -> bool:
    ...
    def _add_entry(
        self, class_id: int, teacher_id: int, lesson_id: int,
        classroom_id: int, day: int, slot: int
    ):
        """Kayıt ekle"""
        entry = {
            'class_id': class_id,
