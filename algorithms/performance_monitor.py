...
class PerformanceMonitor:
    ...
    def timing_decorator(self, func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Log if slow
                if elapsed > 1.0:
                    self.logger.warning(f"Slow function: {func_name} took {elapsed:.2f}s")

                return result

            except Exception as e:
                elapsed = time.time() - start
                self.logger.error(f"Function {func.__name__} failed after {elapsed:.2f}s: {e}")
                raise
    ...
    def export_metrics_json(self, filename: str = "performance_metrics.json"):
        ...
        data = {
            ...
            "raw_metrics": {func: times for func, times in self.metrics.items()},
        }
