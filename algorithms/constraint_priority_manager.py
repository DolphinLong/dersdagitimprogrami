...
class ConstraintPriorityManager:
    ...
    def set_priority(self, constraint_id: str, 
                    priority: ConstraintPriority) -> bool:
        """
        ...
        for constraint_id, constraint_info in self.DEFAULT_CONSTRAINTS.items():
            result[constraint_id] = {
                ...
                'current_priority': self.priorities.get(
                    constraint_id, 
                    constraint_info['default_priority']
                ).name
            }
    ...
    def get_constraints_by_priority(self, 
                                   priority: ConstraintPriority) -> List[str]:
        """
        ...
        """
        return [
            cid for cid, p in self.priorities.items() 
            if p == priority
        ]
    ...
    def should_enforce(self, constraint_id: str, 
            ...
            for cid, priority_name in priorities_data.items():
                if cid in self.DEFAULT_CONSTRAINTS:
                    try:
                        ...
                        self.logger.warning(
                            f"Invalid priority '{priority_name}' for {cid}"
                        )
    ...
    def validate_priorities(self) -> List[str]:
        ...
        for cid in critical_constraints:
            if self.priorities.get(cid) != ConstraintPriority.CRITICAL:
                warnings.append(
                    f"Warning: {cid} should be CRITICAL for valid schedules"
                )
        
        # Check if too many constraints are OPTIONAL
        optional_count = sum(
            1 for p in self.priorities.values() 
            if p == ConstraintPriority.OPTIONAL
        )
