...
class SchedulingFailure:
    ...
    def __init__(self, lesson_name: str, class_name: str, 
                 teacher_name: str, required_hours: int,
                 scheduled_hours: int, reason: str, 
                 context: Dict = None):
    ...
    def __repr__(self):
        return (f"Failure({self.class_name}-{self.lesson_name}: "
...
class SchedulerExplainer:
    ...
    def log_failure(self, lesson_name: str, class_name: str,
                   teacher_name: str, required_hours: int,
                   scheduled_hours: int, reason: str,
                   context: Dict = None):
        """Bir başarısızlığı kaydet"""
        failure = SchedulingFailure(
            lesson_name, class_name, teacher_name,
            required_hours, scheduled_hours, reason, context
        )
    ...
    def _identify_critical_issues(self) -> List[str]:
        ...
        # 3. Çakışma fazlalığı
        conflict_count = (self.stats[self.REASON_TEACHER_CONFLICT] + 
                         self.stats[self.REASON_CLASS_CONFLICT])
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.stats[self.REASON_DOMAIN_EXHAUSTED] > 0:
            recommendations.append(
                "💡 Bazı derslerin haftalık saat sayısını azaltmayı düşünün."
            )
    ...
    def generate_report(self) -> str:
        ...
        if analysis['reasons']:
            ...
            for reason, count in sorted(analysis['reasons'].items(), 
                                       key=lambda x: -x[1]):
        ...
        if self.failures:
            ...
            for i, failure in enumerate(self.failures[:10], 1):
                success_rate = (failure.scheduled_hours / failure.required_hours * 100) \
                              if failure.required_hours > 0 else 0
                report.append(
                ...
                report.append(
                    f"      Neden: {self._get_reason_name(failure.reason)}"
                )
            report.append("")
        ...
        if teacher_loads:
            ...
            sorted_teachers = sorted(teacher_loads.items(), 
                                   key=lambda x: -x[1])[:10]
