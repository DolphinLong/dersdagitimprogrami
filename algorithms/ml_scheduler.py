...
class MLScheduler:
    ...
    def extract_features(self, class_id: int, lesson_id: int, 
                        teacher_id: int, day: int, slot: int,
                        current_schedule: List[Dict]) -> Dict:
        """
        ...
        teacher_hours_today = sum(
            1 for e in current_schedule 
            if e['teacher_id'] == teacher_id and e['day'] == day
        )
        ...
        features['historical_success'] = self._get_historical_success_rate(
            lesson_id, day, slot
        )
        
        return features
    
    def predict_best_slot(self, class_id: int, lesson_id: int, 
                         teacher_id: int, available_slots: List[Tuple[int, int]],
                         current_schedule: List[Dict]) -> Optional[Tuple[int, int]]:
        """
        ...
        if not self.is_trained:
            ...
            return self._heuristic_selection(
                class_id, lesson_id, teacher_id, 
                available_slots, current_schedule
            )
    ...
    def learn_from_schedule(self, schedule: List[Dict], 
    ...
    def train_model(self):
        ...
        if successful_features:
            ...
            self.learned_weights = {
                'morning_preference': sum(f.get('is_morning', 0) for f in successful_features) / len(successful_features),
                'avoid_teacher_overload': 1.0 - (sum(f.get('teacher_is_busy', 0) for f in successful_features) / len(successful_features)),
                'avoid_gaps': 1.0 - (sum(f.get('class_has_gap', 0) for f in successful_features) / len(successful_features)),
    ...
    def _heuristic_selection(self, class_id: int, lesson_id: int,
                            teacher_id: int, available_slots: List[Tuple[int, int]],
                            current_schedule: List[Dict]) -> Tuple[int, int]:
    ...
    def _check_gap(self, schedule: List[Dict], class_id: int, 
                  day: int, slot: int) -> int:
        """Check if placing at this slot would create a gap"""
        class_slots_today = [
            e['time_slot'] for e in schedule 
            if e['class_id'] == class_id and e['day'] == day
        ]
    ...
    def _is_difficult_lesson(self, lesson_name: str) -> int:
        ...
        difficult_lessons = [
            ...
            "Türk Dili ve Edebiyatı", "Geometri"
        ]
        return 1 if lesson_name in difficult_lessons else 0
    
    def _get_historical_success_rate(self, lesson_id: int, 
                                    day: int, slot: int) -> float:
        ...
        for hist in self.historical_schedules:
            for entry in hist['schedule']:
                if (entry['lesson_id'] == lesson_id and 
                    entry['day'] == day and 
