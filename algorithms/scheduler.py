...
class Scheduler:
    ...
    def __init__(self, db_manager, use_advanced=True, use_hybrid=True, use_ultra=True, progress_callback=None):
        ...
        # Fallback to strict scheduler
            ...
            if self.use_advanced:
                ...
                print("✨ Using Advanced Scheduler with smart distribution")
            else:
    ...
    def _generate_schedule_standard(self):
        ...
        # Days of the week (0-4 for Monday-Friday)
        days = list(range(5))
        
        # Time slots based on school type
        time_slots = list(range(time_slots_count))
        
        print(f"Starting schedule generation for {len(classes)} classes, {len(teachers)} teachers, {len(lessons)} lessons")
        print(f"School type: {school_type}, Time slots: {time_slots_count}")
        print(f"Found {len(existing_assignments)} existing lesson assignments")
        ...
        print(f"Created {len(lesson_assignments)} lesson-teacher assignments")
        ...
        for class_obj in classes:
                ...
                if assignment_key in lesson_assignments:
                    ...
                    if weekly_hours and weekly_hours > 0:
                        ...
                        if assigned_teacher:
                            ...
                            print(f"  📋 Found assignment: {lesson.name} -> {assigned_teacher.name} ({weekly_hours} hours)")
            
            if not class_lessons:
                print(f"  ⚠️  No lesson assignments found for {class_obj.name}")
                continue
            ...
            for lesson, weekly_hours, assigned_teacher in class_lessons:
                print(f"Scheduling assigned lesson: {lesson.name} ({weekly_hours} hours) with {assigned_teacher.name}")
                
                # Try to schedule this lesson with the assigned teacher
                success = self._schedule_lesson_with_assigned_teacher(
                    schedule_entries, class_obj, assigned_teacher, lesson,
                    days, time_slots, weekly_hours
                )
                
                if not success:
                    print(f"⚠️  Could not fully schedule {lesson.name} for {class_obj.name} with {assigned_teacher.name}")
            
            # Print class schedule summary
            class_total = len([e for e in schedule_entries if e['class_id'] == class_obj.class_id])
            print(f"✅ Class {class_obj.name} scheduled: {class_total} hours")
        ...
        if conflicts:
            print(f"⚠️  {len(conflicts)} conflicts detected - attempting resolution...")
    ...
    def _schedule_lesson_with_assigned_teacher(self, schedule_entries, class_obj, assigned_teacher, lesson, days, time_slots, weekly_hours):
        """Schedule a lesson with its specifically assigned teacher - AGGRESSIVE MODE"""
        print(f"  🚀 AGGRESSIVE Scheduling {lesson.name} for {class_obj.name} with assigned teacher {assigned_teacher.name}")
        ...
        for attempt in range(max_attempts):
            ...
            if attempt < 10:
                ...
                block_sizes = [remaining_hours] if remaining_hours <= 3 else [2, 1] * (remaining_hours // 2) + [1] * (remaining_hours % 2)
            ...
            for block_size in block_sizes:
                ...
                for day in day_order:
                    ...
                    available_slots = self._find_best_slots_aggressive(schedule_entries, class_obj.class_id, day, time_slots, block_size)
                    
                    if available_slots:
                        # Check if assigned teacher is available for all slots - AGGRESSIVE
                        if self._can_teacher_teach_at_slots_aggressive(schedule_entries, assigned_teacher.teacher_id, day, available_slots):
                            ...
                            for i in range(slots_to_schedule):
                                ...
                                if not self._has_conflict(schedule_entries, new_entry):
                                    ...
                                    print(f"    ✓ AGGRESSIVE: {lesson.name} - Day {day+1}, Slot {time_slot+1} ({assigned_teacher.name})")
                            
                            used_days.add(day)
                            break  # Move to next block
                        else:
                            if attempt < 5:  # Only show warnings for first few attempts
                                print(f"    ⚠️  Teacher {assigned_teacher.name} not available on Day {day+1} for {block_size} slots")
        
        success_rate = (scheduled_hours / weekly_hours) * 100 if weekly_hours > 0 else 100
        if success_rate < 100:
            print(f"    ⚠️  {lesson.name}: {scheduled_hours}/{weekly_hours} hours scheduled ({success_rate:.1f}%)")
        else:
            print(f"    ✅ {lesson.name}: {scheduled_hours}/{weekly_hours} hours scheduled (100%)")
        
        return scheduled_hours >= weekly_hours  # AGGRESSIVE: Only accept 100% success

    def _schedule_lesson_improved(self, schedule_entries, class_obj, specific_teacher, lesson, days, time_slots, weekly_hours, eligible_teachers=None):
        ...
        for attempt in range(max_attempts):
            ...
            for block_size in block_sizes:
                ...
                for day in range(5):
                    ...
                    available_slots = self._find_best_slots(schedule_entries, class_obj.class_id, day, time_slots, block_size)
                    
                    if available_slots:
                        # Try each eligible teacher
                        for teacher in teachers_to_try:
                            if self._can_teacher_teach_at_slots(schedule_entries, teacher.teacher_id, day, available_slots):
                                ...
                                for i in range(slots_to_schedule):
                                    ...
                                    if not self._has_conflict(schedule_entries, new_entry):
                                        schedule_entries.append(new_entry)
                                        scheduled_hours += 1
                                        print(f"  ✓ Scheduled {lesson.name} - Day {day+1}, Slot {time_slot+1} ({teacher.name})")
    ...
    def _create_optimal_blocks_distributed(self, total_hours):
        ...
        if total_hours <= 2:
            ...
            num_days_available = 5
            max_blocks_per_day = 1  # Try to limit to 1 block per day initially
    ...
    def _find_lesson_by_name(self, lessons, subject_name, grade):
        ...
        for lesson in lessons:
            ...
            if (subject_name == "Beden Eğitimi ve Oyun" and lesson.name == "Beden Eğitimi ve Oyun") or \
               (subject_name == "Beden Eğitimi ve Spor" and lesson.name == "Beden Eğitimi ve Spor") or \
    ...
    def _find_available_teacher_for_period(self, schedule_entries, teachers, lesson, day, start_time, end_time):
        ...
        if not eligible_teachers:
            print(f"No teachers available for subject: {lesson.name}")
            return None
