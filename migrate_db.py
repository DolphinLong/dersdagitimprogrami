...
def migrate():
    ...
    try:
        ...
        if "UNIQUE (NAME)" not in schema_sql and "NAME TEXT UNIQUE" not in schema_sql:
            logging.warning(
                f"Beklenmedik şema yapısı. Şema: {schema_sql}. Güvenlik için taşıma işlemi iptal edildi."
            )
            return
        ...
        if "school_type" in columns:
            ...
            logging.warning(
                "'school_type' sütunu eski tabloda bulunamadı. Varsayılan değer 'Lise' olarak atanacak."
            )
            cursor.execute(
                """
                INSERT INTO lessons (lesson_id, name, weekly_hours, school_type)
