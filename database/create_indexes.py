...
class DatabaseIndexManager:
    ...
    def _get_index_definitions(self) -> List[Tuple[str, str]]:
        ...
        return [
            ...
            ("idx_classes_school_type", "CREATE INDEX idx_classes_school_type ON classes(school_type)"),
            ("idx_classes_grade", "CREATE INDEX idx_classes_grade ON classes(grade)"),
            # Teachers table indexes
            ("idx_teachers_subject", "CREATE INDEX idx_teachers_subject ON teachers(subject)"),
            ("idx_teachers_school_type", "CREATE INDEX idx_teachers_school_type ON teachers(school_type)"),
            # Lessons table indexes
            ("idx_lessons_name", "CREATE INDEX idx_lessons_name ON lessons(name)"),
            # Schedule table indexes (most critical for performance)
            ("idx_schedule_class_id", "CREATE INDEX idx_schedule_class_id ON schedule(class_id)"),
            ("idx_schedule_teacher_id", "CREATE INDEX idx_schedule_teacher_id ON schedule(teacher_id)"),
            ("idx_schedule_lesson_id", "CREATE INDEX idx_schedule_lesson_id ON schedule(lesson_id)"),
            ...
            # Curriculum indexes
            ("idx_curriculum_lesson", "CREATE INDEX idx_curriculum_lesson ON curriculum(lesson_id)"),
            ("idx_curriculum_grade", "CREATE INDEX idx_curriculum_grade ON curriculum(grade)"),
            # Users table indexes
    ...
    def analyze_database(self) -> dict:
        ...
        try:
            ...
            analysis = {"tables": {}, "indexes": {}, "recommendations": []}
            ...
            for (table_name,) in tables:
                ...
                analysis["tables"][table_name] = {"rows": row_count}
            ...
            for index_name, table_name, sql in indexes:
                analysis["indexes"][index_name] = {"table": table_name, "sql": sql}
...
def main():
    ...
    parser.add_argument("--db-path", default="schedule.db", help="Path to database file")
    parser.add_argument(
        "--action", choices=["create", "drop", "rebuild", "analyze"], default="create", help="Action to perform"
    )
